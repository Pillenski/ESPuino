...
#include "Audio.h"
#include "AudioPlayer_TrackCtrl.h"
#include "Bluetooth.h"
#include "Cmd.h"
#include "Common.h"
#include "EnumUtils.h"
...
uint32_t lastPlayingTimestamp = 0;
...
// ---------------------------------------------------------------------------------------------------------------------
// AudioPlayer_Task helper functions
// ---------------------------------------------------------------------------------------------------------------------

/**
 * @brief Handles incoming volume changes from gVolumeQueue
 */
static void AudioPlayer_HandleVolumeQueue(Audio *audio) {
...
/**
 * @brief Handles equalizer changes from gEqualizerQueue
 */
static void AudioPlayer_HandleEqualizerQueue(Audio *audio) {
	int8_t currentEqualizer[3];
	if (xQueueReceive(gEqualizerQueue, &currentEqualizer, 0) == pdPASS) {
		Log_Printf(LOGLEVEL_DEBUG, newEqualizerReceivedQueue,
			currentEqualizer[0], currentEqualizer[1], currentEqualizer[2]);
		audio->setTone(currentEqualizer[0], currentEqualizer[1], currentEqualizer[2]);
	}
}

/**
 * @brief Reads control commands (next/prev/pause/...) from gTrackControlQueue
 *
 * The value is written back via reference so that the caller can evaluate it later on.
 */
static void AudioPlayer_HandleTrackControlQueue(uint8_t &trackCommand) {
...
/**
 * @brief Updates current playtime / duration statistics every 250 ms
 */
static void AudioPlayer_UpdatePlaytimeStats(Audio *audio) {
	static uint32_t lastPlaytimeStatsTimestamp = 0;
	if ((millis() - lastPlaytimeStatsTimestamp) > 250u) {
		lastPlaytimeStatsTimestamp = millis();

		// Update current play‑time and duration
		AudioPlayer_CurrentTime = audio->getAudioCurrentTime();
		AudioPlayer_FileDuration = audio->getAudioFileDuration();

		// Calculate relative position in file (for track progress neopixel & Web UI)
		...
		if (!gPlayProperties.playlistFinished && fileSize > 0) {
			...
			if (!gPlayProperties.pausePlay && (gPlayProperties.seekmode != SEEK_POS_PERCENT)) {
				uint32_t audioDataStartPos = audio->getAudioDataStartPos();
				gPlayProperties.currentRelPos = ((double) (audio->getFilePos() - audioDataStartPos + audio->inBufferFilled()) / (fileSize - audioDataStartPos)) * 100.0;
			}
		} else {
			if (gPlayProperties.isWebstream && (audio->inBufferSize() > 0)) {
				// calc current fillbuffer percent for webstream with unknown size/end
				gPlayProperties.currentRelPos = (double) (audio->inBufferFilled()) / (double) audio->inBufferSize() * 100.0;
			} else {
...
/**
 * @brief Handles playlist, track‑finished, and control‑commands logic
 * 
 */
static void AudioPlayer_ProcessTrackEvents(Audio *audio, uint32_t &playbackTimeoutStart, uint8_t &trackCommand) {
	Playlist *newPlaylist = nullptr;
	BaseType_t trackQStatus = xQueueReceive(gTrackQueue, &newPlaylist, 0);
		...
		if (gPlayProperties.saveLastPlayPosition) { // Don't save for AUDIOBOOK_LOOP because not necessary
			if (gPlayProperties.currentTrackNumber + 1 < gPlayProperties.playlist->size()) {
				...
				AudioPlayer_NvsRfidWriteWrapper(
					...
					gPlayProperties.playlist->at(gPlayProperties.currentTrackNumber),
					0,
	...
	if (trackCommand != NO_ACTION) {
		AudioPlayer_TrackCtrl::executeTrackCommand(audio, trackCommand);
		trackCommand = NO_ACTION;
	}
}

/**
 * @brief Calls Audio::loop(), handles activity‑timer and playback‑timeout supervision
 * 
 */
static void AudioPlayer_MainLoopHousekeeping(Audio *audio, uint32_t &playbackTimeoutStart) {
	constexpr uint32_t playbackTimeout = 2000u;
	...
	if (gPlayProperties.playlistFinished || gPlayProperties.pausePlay) {
		...
		System_UpdateActivityTimer(); // Refresh so µC will not fall asleep
	}
...
// RFID reset logic when the box becomes idle -------------------------------
...
// ---------------------------------------------------------------------------------------------------------------------
// Functions to play music as task
// ---------------------------------------------------------------------------------------------------------------------
void AudioPlayer_Task(void *parameter) {
	...
	for (;;) {
		AudioPlayer_HandleVolumeQueue(audio);
		AudioPlayer_HandleEqualizerQueue(audio);
		AudioPlayer_UpdatePlaytimeStats(audio);
		AudioPlayer_HandleTrackControlQueue(trackCommand);
		AudioPlayer_ProcessTrackEvents(audio, playbackTimeoutStart, trackCommand);
		AudioPlayer_MainLoopHousekeeping(audio, playbackTimeoutStart);
	}
	vTaskDelete(NULL);
}
